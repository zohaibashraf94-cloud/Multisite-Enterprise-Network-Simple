...
Action Taken: The access port was manually re-assigned using switchport access vlan 11.
The Final Piece: Although local Inter-VLAN routing became active (allowing successful ICMP pings to gateway 10.1.11.1), DHCP traffic still dropped. An ip helper-address 10.1.10.10 directive was applied to the local HQ router sub-interface Gig0/0/0.11. This allowed the router to correctly hand off the local broadcast to the Server VLAN, achieving complete network convergence.


Branch-Staff-PC> ping 10.1.11.50

Pinging 10.1.11.50 with 32 bytes of data:
